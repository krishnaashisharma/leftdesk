...
// Because the url is always `https://api.leftdesk.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
...
#[inline]
pub fn is_leftdesk() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("LeftDesk")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.leftdesk.com".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    let url = url.to_ascii_lowercase();
    url.contains("leftdesk.com/") || url.ends_with("leftdesk.com")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/LeftDesk.app/Contents/MacOS/LeftDesk", "XPC_SERVICE_NAME=com.carriez.LeftDesk_server"
...
pub type LeftDeskInterval = ThrottledInterval;

#[inline]
pub fn leftdesk_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_LeftDesk_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = leftdesk_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_LeftDesk_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "leftdesk.com/"
        assert!(is_public("https://leftdesk.com/"));
        assert!(is_public("https://www.leftdesk.com/"));
        assert!(is_public("https://api.leftdesk.com/v1"));
        assert!(is_public("https://API.RUSTDESK.COM/v1"));
        assert!(is_public("https://leftdesk.com/path"));

        // Test URLs ending with "leftdesk.com"
        assert!(is_public("leftdesk.com"));
        assert!(is_public("https://leftdesk.com"));
        assert!(is_public("https://LeftDesk.com"));
        assert!(is_public("http://www.leftdesk.com"));
        assert!(is_public("https://api.leftdesk.com"));
        ...
        assert!(!is_public("https://leftdesk.computer.com"));
        assert!(!is_public("leftdesk.comhello.com"));
    }
    ...
    fn test_should_use_tcp_proxy_for_api_url() {
        ...
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.leftdesk.com/api/login",
            "https://admin.leftdesk.com"
        ));
        assert!(!should_use_tcp_proxy_for_api_url(
            "https://admin.example.com/api/login",
            "not a url"
