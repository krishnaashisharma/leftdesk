...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "leftdesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/leftdesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "LeftDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "LeftDesk wants to uninstall the plugin",
    )
}
