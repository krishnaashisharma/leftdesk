...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_leftdesk() {
            if s.contains("LeftDesk")
                && !name.starts_with("upgrade_leftdesk_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("LeftDesk") {
                    s = s.replace("LeftDesk", &app_name);
                } else {
                    // https://github.com/leftdesk/leftdesk-server-pro/issues/845
                    // If app_name contains "LeftDesk" (e.g., "LeftDesk-Admin"), we need to avoid
                    // replacing "LeftDesk" within the already-substituted app_name, which would
                    // cause duplication like "LeftDesk-Admin" -> "LeftDesk-Admin-Admin".
                    //
