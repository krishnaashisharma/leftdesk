...
hbb_name = 'leftdesk' + ('.exe' if windows else '')
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/leftdesk')
    system2('mkdir -p tmpdeb/etc/rustdesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/leftdesk/files/systemd/')
    ...
    system2('dpkg-deb -b tmpdeb leftdesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('leftdesk.deb', '../leftdesk-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibleftdesk.dylib target/release/libleftdesk.dylib")
    os.chdir('flutter')
    system2('flutter build macos --release')
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/LeftDesk.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"LeftDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon LeftDesk.app 200 190 --hide-extension LeftDesk.app leftdesk.dmg ./build/macos/Build/Products/Release/LeftDesk.app")
    os.rename("leftdesk.dmg", f"../leftdesk-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/liblibleftdesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libleftdesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/leftdesk.exe')
    os.chdir('../..')
    if os.path.exists('./leftdesk_portable.exe'):
        os.replace('./target/release/leftdesk-portable-packer.exe',
                   './leftdesk_portable.exe')
    else:
        os.rename('./target/release/leftdesk-portable-packer.exe',
                  './leftdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/leftdesk_portable.exe')
    os.rename('./leftdesk_portable.exe', f'./leftdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/leftdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/leftdesk.exe')
        system2('mv target/release/leftdesk.exe target/release/LeftDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\leftdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/LeftDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
        ...
        if flutter:
            ...
            if osx:
                system2(
                    'strip target/release/bundle/osx/LeftDesk.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/LeftDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LeftDesk.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LeftDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/LeftDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/LeftDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/LeftDesk.app
    '''.format(pa))
                system2(
                    'create-dmg "LeftDesk %s.dmg" "target/release/bundle/osx/LeftDesk.app"' % version)
                os.rename('LeftDesk %s.dmg' %
                          version, 'leftdesk-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    # verify:  spctl -a -t exec -v /Applications/LeftDesk.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
