...
class PlatformFFI {
  ...
  LeftdeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libleftdesk.so')
        : isLinux
            ? DynamicLibrary.open('libleftdesk.so')
            : isWindows
                ? DynamicLibrary.open('libleftdesk.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibleftdesk.dylib") :
  ...
  void _startListenEvent(LeftdeskImpl leftdeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = leftdeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
