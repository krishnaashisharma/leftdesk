...
{
  HINSTANCE hInstance = LoadLibraryA("libleftdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libleftdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN rustdesk_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "leftdesk_core_main_args");
  ...
  std::wstring app_name = L"LeftDesk";
  FUNC_RUSTDESK_GET_APP_NAME get_leftdesk_app_name = (FUNC_RUSTDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_leftdesk_app_name");
  if (get_leftdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_leftdesk_app_name(app_name_buffer, 512) == 0) {
